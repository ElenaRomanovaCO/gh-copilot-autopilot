...
In that case: derive a new slug (step 1) and create its directory (step 3); **archive the finished run** — move `state.js` to `.autopilot/<previous-slug>/state.js`, write a fresh one for this flight and re-open the dashboard (step 3, rules in [`phases/0-instruments.md`](./0-instruments.md)); top up the memory file rather than rewriting it (step 5); close the stage (step 7). Skip the conventions note and the git setup — they are already there, and `.autopilot/README.md` describes the folder, not the run.
...
- **which shell the terminal is** — run `$PSVersionTable.PSVersion` and see whether it answers. PowerShell or bash decides the clock command, the truncation command and the copy command for the whole run (the table is in `SKILL.md`); record the answer in `state.js` as `shell` so a resume does not have to ask again. Settle it here and it is settled: discovered at ticket four, it has already written a column of unparseable timestamps.
...
│   ├── <YYYY-MM-DD>-brief.md
│   ├── manifest.md
│   ├── reference.md        (only if the briefing collects one — [`phases/2-briefing.md`](./2-briefing.md))
│   ├── spec.md
│   ├── interfaces.md
│   └── tickets/
├── state.js
...
The brief carries **the date it was dictated in its filename** — `2026-08-07-brief.md`. A slug directory outlives one conversation: the user comes back a month later with "finish it up", a second brief gets appended, and a file called `brief.md` gives no way to tell which sitting is which. The date is the cheapest possible answer, and it sorts.

`state.js` and `dashboard.html` are written now, empty-but-valid, per [`phases/0-instruments.md`](./0-instruments.md). The initial `stages` array lists all eight stages — `preflight` as `active`, the rest `pending` — so the dashboard shows the whole road from the first minute instead of a blank page.

## 3a. Raise the instruments

Copy the template, write the starting `state.js`, open the page once. **Read [`phases/0-instruments.md`](./0-instruments.md)** — it is Phase 0's whole share of the dashboard, including the update ritual you will use for the rest of the run.

Do **not** read [`phases/7-instruments.md`](./7-instruments.md) here. It answers questions that arrive in Phase 4, and reading it now spends six thousand characters of the one context that is never refreshed.
...
- `dashboard.html` — opens by itself at the start of the build; a double-click works too.
  Stages, progress, time, what's left. Refreshes itself while the build is running.
- `<project>/<date>-brief.md` — your original task, word for word. Never edited.
- `<project>/manifest.md` — the list of requirements and what became of each.
- `<project>/spec.md` — the specification.
- `<project>/tickets/` — the tickets the build was split into (a small build has none).

If the build got interrupted — tell the agent "continue autopilot"; it will pick the state up from here.
```

## 5. Raise the project memory

The repo needs a file that tells the **next** session what this project is — `CLAUDE.md` or `AGENTS.md`. Which one is decided by detection, never by a question; the skeleton is written now and finished in Phase 8. **Read [`phases/0-memory.md`](./0-memory.md)** — the detection table and the skeleton, and nothing else applies until the build is over.
...
Record the chosen file in `state.js` as `memoryFile`, and note it in the Phase 8 report. Do **not** read [`phases/9-memory.md`](./9-memory.md) here — everything in it belongs to Phase 5 and Phase 8.
...
2. Tell the user in one line where things stand: "Continuing: 7 of 12 tickets done, next up — the cart".
