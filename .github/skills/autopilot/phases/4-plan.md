...
**T0 is real and it is common.** A landing page, a form, a script, a single endpoint — cut nothing, build it in one pass, review once, done. Skipping tickets here is not a shortcut; creating them would be the waste. Say so plainly: "The task is small — building it in one pass, no split".

At T0 the `plan` stage in `state.js` is marked `skipped` with the reason as its note — "tier T0 — no ticket split" — not left `pending`. A stage that never moves reads as a stuck build, and the whole point of the dashboard is that it does not have to be explained.
...
```json
{ "id": "04", "title": "Repairman panel: request queue", "requirements": ["R04", "R04.1"],
  "blockedBy": ["02"], "wave": 3, "zone": ["src/admin/"], "status": "pending", "retries": 0 }
```

`status: "pending"`, no timestamps yet — they come when the ticket launches. This is one edit, and it is what turns the dashboard from "tickets not cut yet" into the whole plan with its waves, visible before a line of code is written.

**A build running while the dashboard still says the tickets were never cut is broken instruments**, and it breaks them at the exact moment the user is most likely to look. The count, the waves, the "Build progress" block and the honest progress bar all read from this array — nothing on the dashboard can show what was never written. See [`phases/7-instruments.md`](./7-instruments.md).
...
## What should start working

A client writes to the bot, answers three questions — what broke, the address, the phone —
and gets a confirmation with a request number. If the network dropped halfway,
the next message continues from the same place instead of starting over.

## From the brief, verbatim

> "takes repair requests for appliances"
...
- [ ] The three-step dialogue reaches the confirmation
- [ ] The request number is unique and visible to the client
- [ ] An interrupted dialogue continues instead of resetting
- [ ] A missing phone number gives a clear error, not a crash
- [ ] A test on seam §1 covers the full path and the interruption
```
...
**Parallelism gets one line, and only if it is true**: "Some tickets will run in parallel — 6 tickets in 4 waves". It is the one piece of process the user actually feels, because it changes how long they wait. Never claim it for a plan that is one long chain.

**semi and interview** — one screen, plain language, no technical detail, one line per ticket saying what the user will be able to do when it lands. Then: "Showing the plan and starting. Say 'stop' if something's off". Then start. Do not wait for approval — waiting is the failure mode this skill exists to remove. **Never promise a countdown:** you cannot hold a pause, so a stated delay is a promise you will break. The user's window to object is their own reaction, and saying so plainly is the honest version of it.
