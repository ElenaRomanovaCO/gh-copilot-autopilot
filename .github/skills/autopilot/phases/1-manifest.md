...
| Generic | ≥32 chars of hex or base64 sitting next to `key`, `token`, `secret`, `password`, `credentials`, `access` |
...
3. Tell the user immediately, in one line, in plain language: "You sent a Stripe key — I didn't save it. Put it into `.env` yourself, and better revoke this one and issue a new one: it has been in a conversation."
...
The redacted brief, **word for word**, into `.autopilot/<slug>/<YYYY-MM-DD>-brief.md` — today's date, then `-brief.md`: `2026-08-07-brief.md`. The date is part of the name because a slug directory outlives one sitting; a month later "now finish up this bit too" arrives, and the files have to say which brief came when. Record the chosen name in `state.js` as `briefFile`, so nothing downstream has to guess it.

```markdown
# Original task

> Recorded <date>. This file is never edited — it is the reference
> the finished result is checked against.

<the user's entire text, word for word, after secret redaction>
```
...
- **Everything counts as brief** — the idea, the asides, the constraints, the "oh and it would be nice if", the stack preference, the deadline mentioned in passing.
- **Never edited again.** Later thoughts from the same sitting are appended under `## Addenda`, dated, never merged into the original text. A brief dictated on a **later day** is a new file with that day's date — appending it to an older one erases the fact that the project was asked for twice.
...
Source: `<date>-brief.md`. A row in this list can be removed **only by the user**.

| ID | From the brief (verbatim) | Status | Basis | Where |
|----|---------------------|--------|-----------|-----|
| R01 | "takes repair requests for appliances" | in-ticket | — | spec §2 → T02 |
| R02 | "puts them into a Google Sheet" | in-ticket | — | spec §4 → T05 |
| R03 | "so the client can see the status" | in-spec | — | spec §6 |
| R04 | "and duplicate to SMS" | dropped | user: "no SMS, just Telegram" | — |
...
Mark with a trailing `i` (`R06i`) anything the brief clearly *implies* but never says — "takes requests" implies somewhere to read them; "online store" implies a way to pay.
...
A `D##` row is the only thing allowed to enter the manifest after the briefing, and only from [`phases/5-subagents.md`](./5-subagents.md). It is not a requirement the user made and it never replaces one: a requirement the code proves impossible is a question for the user, not a `D##` that quietly retires it. An idea you had while building is not a discovery either — that is an `A##`, and it lives in the spec under the usual parent-and-proportion rules.

### How fine to cut

One requirement = one thing that can be independently true or false.

- "a Telegram bot that takes requests and puts them into a sheet" → **two** requirements. One can work while the other doesn't.
...
"Took the task apart into 23 requirements — I'll keep them under control to the very end." No table in the chat. The file is the artifact; the chat is a mention of it.
...
**G2 — after the spec.** Two halves, both mandatory, per [`phases/3-spec.md`](./3-spec.md).
...
**G4 — at the final phase.** Blind acceptance, per [`phases/8-final.md`](./8-final.md). Every disagreement between the manifest and the blind verdict goes in the report.
